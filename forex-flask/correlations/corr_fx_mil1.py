...
sql2 = "SELECT * from fx where (day != 20000806 and day != 20010607 and day != 20050507) and day > 20100301 order by a, b, day limit 500000"
...
sql3 = "SELECT * from gdgs1 where SQLDATE > 19990101 and SQLDATE < 20051231 and NumArticles > 5 order by SQLDATE"


mil = pd.read_sql(sql3, cnx)

mil['SQLDATE'] = [datetime.datetime.combine(datetime.date(int(str(time)[0:4]), int(str(time)[4:6]), int(str(time)[6:])), datetime.time()) for time in mil['SQLDATE']]
mil[['SQLDATE']] = mil[['SQLDATE']].astype('datetime64[ns]')
    #p = p.set_index('day')

print mil
mil.to_csv('gdv911.csv', header = False, index= False)
